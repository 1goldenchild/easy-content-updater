...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const rolexEmail = await sendEmail(
      RESEND_API_KEY,
      to,
    ...
    setTimeout(async () => {
      try {
        const kardashianEmail = await sendEmail(
          RESEND_API_KEY,
          to,
            ...
            <body style="margin: 0; padding: 0; background-color: #121212; color: #E5E7EB; font-family: Arial, sans-serif; line-height: 1.6;">
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <div style="background-color: rgba(26, 31, 44, 0.95); border: 1px solid rgba(134, 115, 111, 0.2); border-radius: 8px; padding: 30px; margin-bottom: 30px;">
                  ...
                  <p style="color: #E5E7EB; font-size: 16px; line-height: 1.8; margin-bottom: 20px;">
                    From their brand names to their business ventures, numerology plays a crucial role in their decision-making processes. For instance, the number 6 is often associated with harmony and balance, which is reflected in their family dynamics and business strategies.
                  </p>
                  <p style="color: #E5E7EB; font-size: 16px; line-height: 1.8; margin-bottom: 20px;">
                    Join us as we delve into the fascinating world of numerology and its impact on the Kardashian brand.
                  </p>
                  <div style="text-align: center; margin-top: 40px;">
                    <a href="https://numerology33.com/" style="background: linear-gradient(to right, #86736f, #a39490); color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: bold;">
                      Discover Your Numerological Path to Success
        ...
        // Schedule Elon Musk email to be sent 1 minute after Kardashian email
        setTimeout(async () => {
          try {
            const muskEmail = await sendEmail(
              RESEND_API_KEY,
              to,
              ...
              <html>
                ...
                <body style="margin: 0; padding: 0; background-color: #121212; color: #E5E7EB; font-family: Arial, sans-serif; line-height: 1.6;">
                  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                    ...
                    <div style="text-align: center; margin-top: 2rem; padding-top: 2rem; border-top: 1px solid rgba(134, 115, 111, 0.2); color: #9CA3AF;">
                      <p>© 2024 Numerology 33. All rights reserved.</p>
                    </div>
                  </div>
                </body>
              </html>`
            ...
            // Schedule Bill Gates email to be sent 1 minute after Elon Musk email
            setTimeout(async () => {
              try {
                const gatesEmail = await sendEmail(
                  RESEND_API_KEY,
                  to,
            ...
            }, 60000); // 1 minute delay after Elon Musk email

          } catch (error) {
            console.error("Error sending Elon Musk email:", error);
          }
        }, 60000); // 1 minute delay after Kardashian email

      } catch (error) {
        console.error("Error sending Kardashian email:", error);
      }
    }, 60000); // 1 minute delay after Rolex email
