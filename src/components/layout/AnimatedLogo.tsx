  ...
  return (
    <div className="flex items-center">
      {word.split('').map((letter, index) => (
        <motion.div
          key={index}
          className="inline-block relative font-bold text-2xl w-[1.5em]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          ...
          <AnimatePresence mode="wait">
            {index <= currentIndex ? (
              <motion.span
                ...
                className="inline-block absolute left-1/2 -translate-x-1/2"
              >
                {letter}
              </motion.span>
            ) : (
              <motion.span
                ...
                className="inline-block absolute left-1/2 -translate-x-1/2 text-[#00ff00] font-matrix"
              >
