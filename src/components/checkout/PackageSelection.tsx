        ...
        {packages.map((pkg) => (
          <div 
            ...
            className={`relative p-8 rounded-xl transition-all transform hover:scale-[1.02] duration-300 ${
              pkg.isBestSelling 
                ? 'bg-gradient-to-br from-[#1A1F2C]/95 to-[#2A2F3C]/95 border-2 border-[#FFB547]/30 shadow-xl shadow-[#FFB547]/20' 
                : 'bg-[#1A1F2C]/90 border border-[#403E43] hover:border-[#FFB547]/30 hover:shadow-lg hover:shadow-[#FFB547]/10'
            }`}
          >
            {pkg.isBestSelling && (
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-gradient-to-r from-[#FFB547] via-[#F5A524] to-[#FFB547] rounded-full text-[#1A1F2C] text-sm font-semibold shadow-lg animate-pulse">
            ...
            <div className="flex items-start mb-5">
              <Package className={`w-6 h-6 ${pkg.isBestSelling ? 'text-[#FFB547]' : 'text-[#8E9196]'} mr-3 mt-1`} />
              <div>
                <h3 className={`text-lg font-bold ${pkg.isBestSelling ? 'bg-gradient-to-r from-[#FFB547] via-[#F5A524] to-[#FFB547] bg-clip-text text-transparent' : 'text-[#E5E5E6]'}`}>
                  {pkg.name}
                </h3>
              </div>
            </div>
            
            <p className="text-base text-[#9F9EA1] mb-8">{pkg.description}</p>
                ...
                return (
                  <div 
                    ...
                    className={`flex items-start group hover:transform hover:translate-x-1 transition-transform duration-200 ${
                      pkg.isBestSelling ? 'text-[#FFE7BA]' : 'text-[#C8C8C9]'
                    }`}
                  >
                    <Icon className={`w-5 h-5 ${pkg.isBestSelling ? 'text-[#FFB547] group-hover:text-[#FFE7BA]' : 'text-[#8E9196] group-hover:text-[#9F9EA1]'} mr-3 mt-1 flex-shrink-0 transition-colors duration-200`} />
                    <div className="text-base">
                      <div className="font-medium">{feature.title}</div>
                      <div className="text-sm text-[#8A898C] group-hover:text-[#9F9EA1] transition-colors duration-200">{feature.description}</div>
                    </div>
                  </div>
                )
              })}
            ...
            {pkg.bonus && (
              <div className="mt-8 p-5 rounded-lg bg-gradient-to-br from-[#1A1F2C] to-[#2A2F3C] border border-[#FFB547]/20 relative overflow-hidden group">
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shine" />
                <div className="absolute inset-0 bg-gradient-to-br from-[#FFB547]/5 to-[#F5A524]/5 group-hover:opacity-100 opacity-0 transition-opacity duration-500" />
                <div className="absolute -inset-1 bg-gradient-to-r from-[#FFB547] via-[#F5A524] to-[#FFB547] opacity-20 blur-sm group-hover:opacity-30 transition-opacity duration-500" />
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(255,181,71,0.1),transparent_70%)]" />
                <div className="relative flex items-start">
                  <pkg.bonus.icon className="w-6 h-6 text-[#FFB547] mr-3 mt-1 flex-shrink-0 animate-pulse" />
                  <div>
                    <div className="font-semibold text-base animate-text-shimmer bg-clip-text text-transparent bg-[linear-gradient(110deg,#FFB547,45%,#FFE7BA,55%,#FFB547)] bg-[length:250%_100%]">
                      {pkg.bonus.title}
                    </div>
                    <div className="text-sm text-[#9F9EA1] mt-1">{pkg.bonus.description}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="text-right mt-8">
              {pkg.originalPrice && (
                <span className="text-base line-through text-[#8A898C] mr-3">
                  ${pkg.originalPrice.toFixed(2)}
                </span>
              )}
              <span className={`text-2xl font-bold ${pkg.isBestSelling ? 'animate-text-shimmer bg-clip-text text-transparent bg-[linear-gradient(110deg,#FFB547,45%,#FFE7BA,55%,#FFB547)] bg-[length:250%_100%]' : 'text-white'}`}>
                ${pkg.price.toFixed(2)}
              </span>
            </div>
          </div>
